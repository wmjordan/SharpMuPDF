...
array<Byte>^ Stream::ReadAll(int maxSize) {
...
Stream^ Stream::DecodeTiffFax(int width, int height, int k, bool endOfLine, bool encodeByteAlign, bool endOfBlock, bool blackIs1) {
	auto s = ::DecodeTiffFax(Context::Ptr, _stream, width, height, k, endOfLine, encodeByteAlign, endOfBlock, blackIs1);
	if (s != NULL) {
		return gcnew Stream(s);
	}
	throw MuException::FromContext();
}

Stream::Stream(array<Byte>^ data) {
